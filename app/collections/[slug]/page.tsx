      ...
      <div className="grid md:grid-cols-2 gap-14 items-start">
        ...
        <div className="space-y-3">
          <div className="aspect-square overflow-hidden rounded bg-charcoal/5">
            <SanityImage image={product.mainImage} width={800} height={800}
              className="w-full h-full object-cover" priority />
          </div>
          {product.gallery && (
            <div className="grid grid-cols-3 gap-3">
              {product.gallery.slice(0, 3).map((img, i) => (
                <div key={i} className="aspect-square overflow-hidden rounded bg-charcoal/5">
                  <SanityImage image={img} width={300} height={300} className="w-full h-full object-cover" />
                </div>
              ))}
            </div>
          )}
        ...
        <div className="md:sticky md:top-24">
          <p className="font-body text-[10px] tracking-widest uppercase text-gold mb-3">{product.category}</p>
          <h1 className="font-sans text-4xl font-light text-charcoal mb-4">{product.name}</h1>
          <p className="font-body text-sm text-charcoal/65 leading-relaxed mb-8">{product.shortDescription}</p>

          <div className="border-t border-gold/20 pt-8 mb-8">
            <p className="font-body text-[10px] tracking-widest uppercase text-charcoal/50 mb-4">Price Estimate</p>
            <PriceCalculator
