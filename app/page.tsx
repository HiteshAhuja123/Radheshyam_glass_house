  ...
  return (
    <>
      ...
      <section className="relative min-h-[92vh] bg-charcoal-dark flex items-center justify-center overflow-hidden">
        ...
        <div className="absolute inset-0 grid grid-cols-4 gap-px opacity-[0.07] pointer-events-none">
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="border-x border-gold" />
          ))}
        </div>
        {/* Radial glow */}
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_60%_60%_at_50%_50%,rgba(201,168,76,0.07),transparent)] pointer-events-none" />

        <div className="relative z-10 text-center px-6 max-w-3xl">
          <p className="font-body text-[10px] tracking-[0.22em] uppercase text-gold mb-5 hero-line-1">
            31 Years of Artisan Glass Excellence
          </p>
          <h1 className="font-sans text-white text-5xl md:text-7xl leading-[1.1] font-light mb-6 hero-line-2">
            Where Reflection<br />Becomes{" "}
            <em className="shimmer-text italic">Fine Art</em>
          </h1>
          <p className="font-body text-white/50 text-sm leading-relaxed mb-10 max-w-md mx-auto hero-line-3">
            Designer mirrors and bespoke glass solutions crafted across three generations for
            India's finest homes, hotels, and offices.
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center hero-line-4">
        ...
        {/* Bottom fade */}
        <div className="absolute bottom-0 left-0 right-0 h-24 bg-gradient-to-t from-charcoal-dark/80 to-transparent pointer-events-none" />
      </section>

      {/* ── LEGACY STRIP ── */}
      <div className="bg-gold py-5 px-6">
        <div className="max-w-4xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-6 text-charcoal">
          {[["31+", "Years of legacy"], ["3rd", "Generation craftsmanship"], ["50,000+", "Projects delivered"], ["100%", "Custom made"]].map(([num, label]) => (
            ...
            {products.map((p, i) => (
              <ScrollReveal key={p._id} delay={i * 80}>
                <Link href={`/collections/${p.slug.current}`}
                  className="group bg-white border border-gold/20 rounded overflow-hidden hover:border-gold/50 hover:shadow-md transition-all duration-300">
          ...
          <div className="max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              {galleryItems.slice(0, 6).map((item, i) => (
                <ScrollReveal key={item._id} delay={i * 60} className={i === 0 ? "md:row-span-2" : ""}>
                  <div className="overflow-hidden rounded bg-charcoal/5 h-full">
